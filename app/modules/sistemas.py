...
def render() -> None:
    ...
    if n_variaveis == 2 and abs(np.linalg.det(a)) > 1e-9:
        ...
        st.markdown(f"**Solução numérica (NumPy):** x = {solucao_numerica[0]:.4g}, y = {solucao_numerica[1]:.4g}")

    if mostrar_passo_a_passo:
        mostrar_passos(passos)

    if n_variaveis == 2:
        st.subheader("Interpretação gráfica")
        equacoes = [(a[0, 0], a[0, 1], b[0]), (a[1, 0], a[1, 1], b[1])]
        st.plotly_chart(figura_retas_2d(equacoes), width="stretch")

        st.subheader("Ver a reta e a interseção a variar em tempo real")
